...
use helios_core::execution::providers::freezer::block_freezer::BlockFreezer;
...
impl<DB: Database> EthereumClientBuilder<DB> {
    ...
    pub fn build(self) -> Result<EthereumClient> {
        ...
        let archive_from = self.config.as_ref().and_then(|config| config.archive_from);
        ...
        if let Some(archive_from) = config.archive_from {
            ...
            if let Some(verifiable_api) = &config.verifiable_api {
                ...
                if let Err(e) = block_freezer.start_sync(Arc::clone(&execution)) {
                    warn!("Failed to start block freezer sync: {}", e);
                }
            ...
            if let Some(verifiable_api) = &config.verifiable_api {
                let execution = Arc::new(VerifiableApiExecutionProvider::with_historical_provider(
                    verifiable_api,
                    block_provider,
                    Eip2935Provider::new(),
                ));
