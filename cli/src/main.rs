...
struct EthereumArgs {
    ...
    #[arg(
        long = "freezer-archive-from",
        env,
        help = "Enable block freezer and start archiving from this block number"
    )]
    freezer_archive_from: Option<u64>,
}
...
impl EthereumArgs {
    ...
    fn as_cli_config(&self) -> CliConfig {
        CliConfig {
            ...
            archive_from: self.freezer_archive_from,
        }
    }
}
...
impl OpStackArgs {
    ...
    fn as_provider(&self) -> Serialized<HashMap<&str, Value>> {
        ...
        if let Some(data_dir) = &self.data_dir {
            user_dict.insert("data_dir", Value::from(data_dir.clone()));
        }

        if let Some(archive_from) = self.freezer_archive_from {
            user_dict.insert("archive_from", Value::from(archive_from));
        }
