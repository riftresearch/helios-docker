...
use crate::execution::{
    ...
    providers::{freezer::block_freezer::BlockFreezer, historical::HistoricalBlockProvider},
};
...
pub struct RpcExecutionProvider<N: NetworkSpec, B: BlockProvider<N>, H: HistoricalBlockProvider<N>>
{
    ...
    freezer_provider: Option<Arc<BlockFreezer<N, H>>>,
}
    ...
    RpcExecutionProvider<N, B, H>
{
    ...
    pub fn with_freezer_provider(
        rpc_url: Url,
        block_provider: B,
        historical_provider: H,
        freezer_provider: Arc<BlockFreezer<N, H>>,
    ) -> Self {
        let client = ClientBuilder::default()
            .layer(RetryBackoffLayer::new(100, 50, 300))
            .http(rpc_url);
        ...
        Self {
            ...
            block_provider,
            historical_provider: Some(historical_provider),
            freezer_provider: Some(freezer_provider),
        }
    }
    ...
    async fn get_block(
        ...
        if let Some(historical) = &self.historical_provider {
            if super::utils::should_use_historical_provider(&block_id) {
                // Historical RPCs can be flaky; log errors but treat them as "no historical block"
                match historical
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::debug!(
                            target: "helios::execution::rpc",
                            "historical provider error for {:?}: {}",
                            block_id,
                            e
                        );
                    }
        ...
        if let Some(freezer) = &self.freezer_provider {
            if super::utils::should_use_historical_provider(&block_id) {
                match freezer.get_block(block_id, full_tx, self).await {
                    Ok(Some(block)) => {
                        // Note: Do NOT cache freezer blocks to avoid interfering with consistency detection
                        return Ok(Some(block));
                    }
                    Ok(None) => { /* nothing to do */ }
                    Err(e) => {
